...
import type { Firestore, DocumentReference, DocumentSnapshot, QuerySnapshot, QueryDocumentSnapshot, Transaction, WriteResult, CollectionReference, Query } from '@google-cloud/firestore';
...
function createMockFirestore(): Firestore {
  ...
  const mockCollection = (name: string): CollectionReference => ({
    ...
    limit: vi.fn(function () { return this as unknown as Query; }),
    get: vi.fn(async () => {
      const allDocs: QueryDocumentSnapshot[] = [];
      docs.forEach((data, docId) => {
        if (docId.startsWith(name) || true) {
          allDocs.push({
            id: docId,
            data: () => data,
            exists: true,
    ...
    runTransaction: vi.fn(async (updateFn: (t: Transaction) => Promise<void>) => {
      const transaction = {
        get: vi.fn(async (docRef: DocumentReference) => {
          ...
          return { exists: !!data, data: () => data ?? undefined, id: docRef.id } as DocumentSnapshot;
        }),
...
describe('FirestoreAdapter', () => {
  ...
  it('should not overwrite with older version', async () => {
    ...
    await adapter.saveState(stateV2 as any);

    const stateV1 = makeState({ version: 1, failure_count: 1 });
    await adapter.saveState(stateV1 as any);
  ...
  it('should throw on connect error', async () => {
    const failingFirestore = {
      ...
      collection: vi.fn(() => ({
        limit: vi.fn(() => ({
          get: vi.fn(async () => { throw new Error('connection failed'); }),
        })),
        doc: vi.fn(),
      })),
    } as unknown as Firestore;
  ...
  it('should return unhealthy from healthCheck on error', async () => {
    const failingFirestore = {
      ...
      collection: vi.fn(() => ({
        limit: vi.fn(() => ({
          get: vi.fn(async () => { throw new Error('health check failed'); }),
        })),
        doc: vi.fn(),
      })),
    } as unknown as Firestore;
  ...
  it('should filter invalid data in loadAll', async () => {
    await adapter.saveState(makeState({ circuit_id: 'valid' }) as any);
  ...
  it('should allow leader takeover after lease expires', async () => {
    ...
    const leaderCol = (mockFirestore.collection as ReturnType<typeof vi.fn>)('circuit_breaker_leaders');
